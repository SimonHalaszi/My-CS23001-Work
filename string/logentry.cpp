...
// REQUIRES:
// ENSURES:
// Sets the values for LogEntry given a line from the log files. Empty Constructor called before this one runs.
LogEntry::LogEntry(const String& line) : LogEntry() {
    // Intial Split
...
std::vector<LogEntry> parse(std::istream& in) {
    ...
    while(in.getline(temp, 300)) {
        result.push_back(LogEntry(String(temp)));
    }
...
void output_all(std::ostream& out, const std::vector<LogEntry>& logsVector) {
    for(size_t i = 0; i < logsVector.size(); ++i) {
        out << "LogEntry #" << i + 1 << std::endl;
        out << logsVector[i] << std::endl;
    }
}
...
void by_host(std::ostream& out, const std::vector<LogEntry>& logsVector) {   
    for(size_t i = 0; i < logsVector.size(); ++i) {
        out << "LogEntry #" << i + 1 << std::endl;
        out << logsVector[i].getHost() << std::endl << std::endl;
    }
}
